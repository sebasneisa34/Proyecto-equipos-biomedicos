...
public class JDBCTechEquipmentRepository implements GenericRepository<TechEquipment> {
    ...
    private TechEquipment map(ResultSet result) throws SQLException {
        ...
        try (
            ...
            try (ResultSet result = ps.executeQuery()) {
                if (result.next()) {
                    ...
                    entity = createTechEquipment(id, entity);
                }
            } catch (Exception e) {
    ...
    private TechEquipment createTechEquipment(String equipmentId, TechEquipment entity) {
        final String sqlQuery = "INSERT INTO tech_equipment "
                + "(id, os, ramGB) "
                + "VALUES (?, ?, ?) "
                + "RETURNING id";
        ...
        try (
            ...
            ps.setString(2, entity.getOs());
            ps.setInt(3, entity.getRamGB());

            try (ResultSet result = ps.executeQuery()) {
                if (result.next()) {
                    String id = result.getObject("id", String.class);

                    entity = new TechEquipment(
                            id, entity.getSerial(), entity.getBrand(), entity.getModel(),
                            entity.getType(), entity.getStatus(),
                            entity.getProvider(), entity.getImagePath(), entity.getRamGB());
                }
            } catch (Exception e) {
                throw new SQLException("No se generó ID", e);
            }
